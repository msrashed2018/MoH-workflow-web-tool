...
export class BonesRevealComponent implements OnInit {
  ...
  searchByKey(event: Event) {
    ...
    this.requestService.searchByStatesAndSearchKey("CONTINUE_REGISTERING_DONE","PENDING_REVEAL","NA",this.searchKey,this.page,PAGINATION_PAGE_SIZE)
    .subscribe(
      result => {
...
this.pages = new Array(0);
  ...
  retriveAllRequests(){
    ...
    this.requestService.retrieveByRequestStates("CONTINUE_REGISTERING_DONE","PENDING_REVEAL","NA",this.page,PAGINATION_PAGE_SIZE)
      .subscribe(
        result => {
          if (typeof result !== 'undefined' && result !== null && result.length !=0) {
            this.noDataFound = false;
