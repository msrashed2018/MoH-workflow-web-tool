...
export class RequestResultsComponent implements OnInit {
  ...
  retrieveRequestsBySearchKey(){
    this.requestService.retrieveRequestsBySearchKey(this.searchKey, this.page, RESULTS_PAGE_SIZE)
    .subscribe(
      result => {
  ...
  retriveAllRequests() {
    ...
    this.errorMessage = false;
    this.noDataFound = false;
    this.requestService.retrieveAllRequests(this.page, RESULTS_PAGE_SIZE)
      .subscribe(
        result => {
          if (typeof result !== 'undefined' && result !== null && result['content'].length != 0) {
            this.noDataFound = false;
