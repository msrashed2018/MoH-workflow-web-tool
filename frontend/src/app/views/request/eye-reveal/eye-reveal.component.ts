...
export class EyeRevealComponent implements OnInit {
  ...
  searchByStatesAndSearchKey() {
    this.requestService.searchByStatesAndSearchKey("CONTINUE_REGISTERING_DONE", "NA", "PENDING_REVEAL", this.searchKey, this.page, GENERAL_PAGE_SIZE)
      .subscribe(
        result => {
  ...
  retriveAllRequests() {
    ...
    this.requestService.retrieveByRequestStates("CONTINUE_REGISTERING_DONE", "NA", "PENDING_REVEAL", this.page, GENERAL_PAGE_SIZE)
      .subscribe(
        result => {
          if (typeof result !== 'undefined' && result !== null && result['content'].length != 0) {
            this.noDataFound = false;
