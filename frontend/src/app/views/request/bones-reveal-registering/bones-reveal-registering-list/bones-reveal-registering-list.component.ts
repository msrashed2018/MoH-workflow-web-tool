...
export class BonesRevealRegisteringListComponent implements OnInit {
  ...
  searchByStatesAndSearchKey() {
    this.requestService.searchByStatesAndSearchKey("CONTINUE_REGISTERING_DONE", "PENDING_REGISTERING", "NA", this.searchKey, this.page, GENERAL_PAGE_SIZE)
      .subscribe(
        result => {
  ...
  retriveAllRequests() {
    ...
    this.requestService.retrieveByRequestStates("CONTINUE_REGISTERING_DONE", "PENDING_REGISTERING", "NA", this.page, GENERAL_PAGE_SIZE)
      .subscribe(
        result => {
          if (typeof result !== 'undefined' && result !== null && result['content'].length != 0) {
            this.noDataFound = false;
